...
export default function PublicTrip() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_BASE}/trips/share/${slug}`).then(r => r.ok ? r.json() : Promise.reject()),
      fetch(`${API_BASE}/trips/share/${slug}/budget`).then(r => r.ok ? r.json() : Promise.reject())
    ])
              ...
              <div className="timeline-view" style={{ marginTop: 8 }}>
                {(() => {
                  if (!trip.start_date || !trip.end_date) {
                    return <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>No dates are set for this trip.</p>;
                  }
